...
onMounted(() => {
  ...
  // Juste vérifier que les tokens existent
  // On les utilisera dans l'API, pas pour créer une session ici
  hasValidToken.value = true;
  console.log("✅ Valid recovery tokens found, ready to reset password");
  isCheckingToken.value = false;
});
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // Envoyer les tokens et le nouveau password à l'API
    const { success, message } = await $fetch("/api/auth/reset-password", {
      method: "POST",
      body: {
        accessToken,
